...
$sqlins = "INSERT INTO sabato (inizio1,inizio2,inizio3,fine1,fine2,fine3) VALUES ('$in1','$in2','$in3','$fi1','$fi2','$fi3')";
...
if ($revinizios1 == '' || $revinizios1 == ' ') {
   ...
   $lines[31] = $revinizios1 . ' * * 6 /bin/sh /tmp/launch.sh';
}
if ($revfines1 == '' || $revfines1 == ' ') {
   $lines[32] = '#32 - Sabato Fine';
} else {
   $lines[32] = $revfines1 . ' * * 6 /bin/sh /tmp/launch.sh';
}
if ($revinizios2 == '' || $revinizios2 == ' ') {
   $lines[33] = '#33 - Sabato Inizio';
} else {
   $lines[33] = $revinizios2 . ' * * 6 /bin/sh /tmp/launch.sh';
}
if ($revfines2 == '' || $revfines2 == ' ') {
   $lines[34] = '#34 - Sabato Fine';
} else {
   $lines[34] = $revfines2 . ' * * 6 /bin/sh /tmp/launch.sh';
}
if ($revinizios3 == '' || $revinizios3 == ' ') {
   $lines[35] = '#35 - Sabato Inizio';
} else {
   $lines[35] = $revinizios3 . ' * * 6 /bin/sh /tmp/launch.sh';
}
if ($revfines3 == '' || $revfines3 == ' ') {
   $lines[36] = '#36 - Sabato Fine';
} else {
   $lines[36] = $revfines3 . ' * * 6 /bin/sh /tmp/launch.sh';
}
