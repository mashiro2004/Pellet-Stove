...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
		...
		// Reverse inizio1
		$revinizio1= substr($row["inizio1"], -2) . ' ' . substr($row["inizio1"], 0, -3);
       	// Reverse fine1
		$revfine1= substr($row["fine1"], -2) . ' ' . substr($row["fine1"], 0, -3);
		// Reverse inizio2
		$revinizio2= substr($row["inizio2"], -2) . ' ' . substr($row["inizio2"], 0, -3);
		// Reverse fine2
		$revfine2= substr($row["fine2"], -2) . ' ' . substr($row["fine2"], 0, -3);
		// Reverse inizio3
		$revinizio3= substr($row["inizio3"], -2) . ' ' . substr($row["inizio3"], 0, -3);
		// Reverse fine3
		$revfine3= substr($row["fine3"], -2) . ' ' . substr($row["fine3"], 0, -3);
	      
    }
//echo $revinizio1, ' ' , $revfine1 . ' ' . $revinizio2, ' ' , $revfine2 . ' ' . $revinizio3, ' ' , $revfine3;
