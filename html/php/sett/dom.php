...
$sqlins = "INSERT INTO domenica (inizio1,inizio2,inizio3,fine1,fine2,fine3,alza,abbassa) VALUES ('$in1','$in2','$in3','$fi1','$fi2','$fi3','$al','$abb')";
...
if ($reviniziod1 == '' || $reviniziod1 == ' ') {
   ...
   $lines[37] = $reviniziod1 . ' * * 7 /bin/sh /var/www/launcher/on.sh';
}
if ($revfined1 == '' || $revfined1 == ' ') {
   $lines[38] = '#38 - Domenica Fine';
} else {
   $lines[38] = $revfined1 . ' * * 7 /bin/sh /var/www/launcher/on.sh';
}
if ($reviniziod2 == '' || $reviniziod2 == ' ') {
   $lines[39] = '#39 - Domenica Inizio';
} else {
   $lines[39] = $reviniziod2 . ' * * 7 /bin/sh /var/www/launcher/on.sh';
}
if ($revfined2 == '' || $revfined2 == ' ') {
   $lines[40] = '#40 - Domenica Fine';
} else {
   $lines[40] = $revfined2 . ' * * 7 /bin/sh /var/www/launcher/on.sh';
}
if ($reviniziod3 == '' || $reviniziod3 == ' ') {
   $lines[41] = '#41 - Domenica Inizio';
} else {
   $lines[41] = $reviniziod3 . ' * * 7 /bin/sh /var/www/launcher/on.sh';
}
if ($revfined3 == '' || $revfined3 == ' ') {
   $lines[42] = '#42 - Domenica Fine';
} else {
   $lines[42] = $revfined3 . ' * * 7 /bin/sh /var/www/launcher/on.sh';
}
if ($revabbassa == '' || $revabbassa == ' ') {
   $lines[55] = '#55 - Domenica Abbassa';
} else {
   $lines[55] = $revabbassa . ' * * 7 /bin/sh /var/www/launcher/10down.sh';
}
if ($revalza == '' || $revalza == ' ') {
   $lines[56] = '#56 - Domenica Alza';
} else {
   $lines[56] = $revalza . ' * * 7 /bin/sh /var/www/launcher/10up.sh';
}
