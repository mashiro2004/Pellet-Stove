...
$sqlins = "INSERT INTO venerdi (inizio1,inizio2,inizio3,fine1,fine2,fine3,alza,abbassa) VALUES ('$in1','$in2','$in3','$fi1','$fi2','$fi3','$al','$abb')";
...
if ($reviniziov1 == '' || $reviniziov1 == ' ') {
   ...
   $lines[25] = $reviniziov1 . ' * * 5 /bin/sh /var/www/launcher/on.sh';
}
if ($revfinev1 == '' || $revfinev1 == ' ') {
   $lines[26] = '#26 - Venerdi Fine';
} else {
   $lines[26] = $revfinev1 . ' * * 5 /bin/sh /var/www/launcher/on.sh';
}
if ($reviniziov2 == '' || $reviniziov2 == ' ') {
   $lines[27] = '#27 - Venerdi Inizio';
} else {
   $lines[27] = $reviniziov2 . ' * * 5 /bin/sh /var/www/launcher/on.sh';
}
if ($revfinev2 == '' || $revfinev2 == ' ') {
   $lines[28] = '#28 - Venerdi Fine';
} else {
   $lines[28] = $revfinev2 . ' * * 5 /bin/sh /var/www/launcher/on.sh';
}
if ($reviniziov3 == '' || $reviniziov3 == ' ') {
   $lines[29] = '#29 - Venerdi Inizio';
} else {
   $lines[29] = $reviniziov3 . ' * * 5 /bin/sh /var/www/launcher/on.sh';
}
if ($revfinev3 == '' || $revfinev3 == ' ') {
   $lines[30] = '#30 - Venerdi Fine';
} else {
   $lines[30] = $revfinev3 . ' * * 5 /bin/sh /var/www/launcher/on.sh';
}
if ($revabbassa == '' || $revabbassa == ' ') {
   $lines[51] = '#51 - Venerdi Abbassa';
} else {
   $lines[51] = $revabbassa . ' * * 5 /bin/sh /var/www/launcher/10down.sh';
}
if ($revalza == '' || $revalza == ' ') {
   $lines[52] = '#52 - Venerdi Alza';
} else {
   $lines[52] = $revalza . ' * * 5 /bin/sh /var/www/launcher/10up.sh';
}
