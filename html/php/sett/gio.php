...
if ($reviniziog1 == '' || $reviniziog1 == ' ') {
   ...
   $lines[19] = $reviniziog1 . ' * * 4 /bin/sh /tmp/launch.sh';
}
if ($revfineg1 == '' || $revfineg1 == ' ') {
   $lines[20] = '#20 - Giovedi Fine';
} else {
   $lines[20] = $revfineg1 . ' * * 4 /bin/sh /tmp/launch.sh';
}
if ($reviniziog2 == '' || $reviniziog2 == ' ') {
   $lines[21] = '#21 - Giovedi Inizio';
} else {
   $lines[21] = $reviniziog2 . ' * * 4 /bin/sh /tmp/launch.sh';
}
if ($revfineg2 == '' || $revfineg2 == ' ') {
   $lines[22] = '#22 - Giovedi Fine';
} else {
   $lines[22] = $revfineg2 . ' * * 4 /bin/sh /tmp/launch.sh';
}
if ($reviniziog3 == '' || $reviniziog3 == ' ') {
   $lines[23] = '#23 - Giovedi Inizio';
} else {
   $lines[23] = $reviniziog3 . ' * * 4 /bin/sh /tmp/launch.sh';
}
if ($revfineg3 == '' || $revfineg3 == ' ') {
   $lines[24] = '#24 - Giovedi Fine';
} else {
   $lines[24] = $revfineg3 . ' * * 4 /bin/sh /tmp/launch.sh';
}
if ($revabbassa == '' || $revabbassa == ' ') {
   $lines[49] = '#49 - Giovedi Abbassa';
} else {
   $lines[49] = $revabbassa . ' * * 4 /bin/sh /tmp/launch.sh';
}
if ($revalza == '' || $revalza == ' ') {
   $lines[50] = '#450 - Giovedi Alza';
} else {
   $lines[50] = $revalza . ' * * 4 /bin/sh /tmp/launch.sh';
}
