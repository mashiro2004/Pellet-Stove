...
$sqlins = "INSERT INTO lunedi (inizio1,inizio2,inizio3,fine1,fine2,fine3,alza,abbassa) VALUES ('$in1','$in2','$in3','$fi1','$fi2','$fi3','$al','$abb')";
...
if ($reviniziol1 == '' || $reviniziol1 == ' ') {
   ...
   $lines[1] = $reviniziol1 . ' * * 1 /bin/sh /var/www/launcher/on.sh';
}
if ($revfinel1 == '' || $revfinel1 == ' ') {
   $lines[2] = '# 2 - Lunedi Fine';
} else {
   $lines[2] = $revfinel1 . ' * * 1 /bin/sh /var/www/launcher/on.sh';
}
if ($reviniziol2 == '' || $reviniziol2 == ' ') {
   $lines[3] = '# 3 - Lunedi Inizio';
} else {
   $lines[3] = $reviniziol2 . ' * * 1 /bin/sh /var/www/launcher/on.sh';
}
if ($revfinel2 == '' || $revfinel2 == ' ') {
   $lines[4] = '# 4 - Lunedi Fine';
} else {
   $lines[4] = $revfinel2 . ' * * 1 /bin/sh /var/www/launcher/on.sh';
}
if ($reviniziol3 == '' || $reviniziol3 == ' ') {
   $lines[5] = '# 5 - Lunedi Inizio';
} else {
   $lines[5] = $reviniziol3 . ' * * 1 /bin/sh /var/www/launcher/on.sh';
}
if ($revfinel3 == '' || $revfinel3 == ' ') {
   $lines[6] = '# 6 - Lunedi Fine';
} else {
   $lines[6] = $revfinel3 . ' * * 1 /bin/sh /var/www/launcher/on.sh';
}
if ($revabbassa == '' || $revabbassa == ' ') {
   $lines[43] = '#43 - Lunedi Abbassa';
} else {
   $lines[43] = $revabbassa . ' * * 1 /bin/sh /var/www/launcher/10down.sh';
}
if ($revalza == '' || $revalza == ' ') {
   $lines[44] = '#44 - Lunedi Alza';
} else {
   $lines[44] = $revalza . ' * * 1 /bin/sh /var/www/launcher/10up.sh';
}
...
<body>
<h1 class="my-3"> </h1>
<p style="font-size:50px; color:green; "> Programmazione del Lunedì </p>
<p>
