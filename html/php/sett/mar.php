...
$sqlins = "INSERT INTO martedi (inizio1,inizio2,inizio3,fine1,fine2,fine3,alza,abbassa) VALUES ('$in1','$in2','$in3','$fi1','$fi2','$fi3','$al','$abb')";
...
if ($reviniziom1 == '' || $reviniziom1 == ' ') {
   ...
   $lines[7] = $reviniziom1 . ' * * 2 /bin/sh /var/www/launcher/on.sh';
}
if ($revfinem1 == '' || $revfinem1 == ' ') {
   $lines[8] = '# 8 - Martedi Fine';
} else {
   $lines[8] = $revfinem1 . ' * * 2 /bin/sh /var/www/launcher/on.sh';
}
if ($reviniziom2 == '' || $reviniziom2 == ' ') {
   $lines[9] = '# 9 - Martedi Inizio';
} else {
   $lines[9] = $reviniziom2 . ' * * 2 /bin/sh /var/www/launcher/on.sh';
}
if ($revfinem2 == '' || $revfinem2 == ' ') {
   $lines[10] = '#10 - Martedi Fine';
} else {
   $lines[10] = $revfinem2 . ' * * 2 /bin/sh /var/www/launcher/on.sh';
}
if ($reviniziom3 == '' || $reviniziom3 == ' ') {
   $lines[11] = '#11 - Martedi Inizio';
} else {
   $lines[11] = $reviniziom3 . ' * * 2 /bin/sh /var/www/launcher/on.sh';
}
if ($revfinem3 == '' || $revfinem3 == ' ') {
   $lines[12] = '#12 - Martedi Fine';
} else {
   $lines[12] = $revfinem3 . ' * * 2 /bin/sh /var/www/launcher/on.sh';
}
if ($revabbassa == '' || $revabbassa == ' ') {
   $lines[45] = '#45 - Martedì Abbassa';
} else {
   $lines[45] = $revabbassa . ' * * 2 /bin/sh /var/www/launcher/10down.sh';
}
if ($revalza == '' || $revalza == ' ') {
   $lines[46] = '#46 - Martedì Alza';
} else {
   $lines[46] = $revalza . ' * * 2 /bin/sh /var/www/launcher/10up.sh';
}
